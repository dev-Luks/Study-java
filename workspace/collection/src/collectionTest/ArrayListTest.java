...
////		설계할 때에는 타입을 지정할 수 없기 때문에 임시로 타입을 선언할 때 사용한다.		
////		따로 다운 캐스팅을 할 필요가 없으며, 지정할 타입에 제한을 줄 수 있다.
//		ArrayList<Integer> datas = new ArrayList<>();  //int는 못씀! 기본자료구조는 쓸수 없고 클래스만 가능
////		System.out.println(datas.size()); //size는 크기, 추가할때마다 늘어날 수 있다.
//		final int SIZE = 9;
...
//		}
//		System.out.println("===========");
//		Collections.sort(datas);   // 오름차순 정령,  주소값이기 때문에 가능 
//		Collections.reverse(datas); // 좌우를 반대로 바꿔주는 메소드
//		Collections.shuffle(datas); //랜덤	
//	} 
//	추가(삽입)
//	50 뒤에 500삽입
